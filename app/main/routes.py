...
def index():
    ...
    posts = current_user.followed_posts().paginate(pages, current_app.config['POSTS_PER_PAGE'], True )
...
def explore():
    pages = request.args.get('page', 1, type=int)
    posts = Post.query.order_by(Post.timestamp.desc()).paginate(pages, current_app.config['POSTS_PER_PAGE'], True )
...
def user(username):
    ...
    posts = user.posts.order_by(Post.timestamp.desc()).paginate(pages, current_app.config['POSTS_PER_PAGE'], False)
